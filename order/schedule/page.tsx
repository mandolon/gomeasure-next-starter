...
function Picker({ scope, title }: { scope: ScopeKey; title: string }) {
  ...
  const days = useMemo(() => {
    ...
    const cells = [];
    // headers
    weekdays.forEach(() => cells.push({ type: "head" } as const));
    // blanks
    for (let i=0;i<startOffset;i++) cells.push({ type: "blank" } as const);
    // month days
    for (let d=1; d<=last.getDate(); d++) cells.push({ type:"day", date: new Date(view.getFullYear(), view.getMonth(), d) } as const);
    return cells as Array<{ type:"head"|"blank"|"day"; date?: Date }>;
  }, [view]);
  ...
  function pickDate(date: Date) {
    ...
    setSchedule(scope, { dateISO: iso, time: null }); // reset time until chosen
  }
